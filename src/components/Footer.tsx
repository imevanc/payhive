    ...
    >
      <div className="mx-auto max-w-10xl">
        <div className="px-6 py-12 lg:px-8">
          ...
          <nav
            aria-label="Footer"
            className="mb-8 flex flex-wrap justify-center gap-x-8 gap-y-3"
          >
            {navigation.main.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-lg font-semibold transition-colors px-2 py-1 rounded text-gray-600 hover:text-green-700"
              >
                {item.name}
              </Link>
            ))}
          ...
          <div className="mb-8 flex justify-center gap-x-6">
            {navigation.social.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-gray-600 transition-colors p-2 rounded-lg group"
              >
                <span className="sr-only">{item.name}</span>
                <item.icon
                  aria-hidden="true"
                  className="size-6 group-hover:text-green-700"
                />
              </Link>
            ))}
          </div>
          ...
          <div className="border-t border-gray-200 pt-8 w-full">
            <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-between">
              ...
              <p className="text-sm text-gray-600">
                &copy; {getCurrentYear()} PayHive Ltd. All rights reserved.
              </p>
            </div>
          </div>
        </div>
